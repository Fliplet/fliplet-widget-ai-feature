...
class CodeAnalyzer {
  ...
  analyzeHTML(html) {
    ...
    if (!html || html.trim() === "") {
      return { elements: [], forms: [], components: [], ids: [], classes: [] };
    }
    ...
    try {
      ...
      console.log("✅ [CodeAnalyzer] HTML analysis complete:", analysis);
      return analysis;
    } catch (error) {
  ...
  analyzeJS(js) {
    ...
    try {
      ...
      const eventMatches = js.match(/addEventListener\s*\(\s*['"`](\w+)['"`]/g);
      if (eventMatches) {
      ...
      console.log("✅ [CodeAnalyzer] JavaScript analysis complete:", analysis);
      return analysis;
    } catch (error) {
      console.error("❌ [CodeAnalyzer] JavaScript analysis failed:", error);
      return { functions: [], eventHandlers: [], variables: [] };
    }
  }
  ...
  findCrossReferences(htmlAnalysis, cssAnalysis, jsAnalysis) {
    ...
    console.log("✅ [CodeAnalyzer] Cross-references complete:", crossRefs);
    return crossRefs;
  }
}
...
class ContextBuilder {
  ...
  buildContext(userMessage, currentCode, changeHistory) {
    ...
    const htmlAnalysis = this.codeAnalyzer.analyzeHTML(currentCode.html);
    ...
    const context = {
      ...
      codeStructure: {
        ...
        jsPatterns: jsAnalysis.functions.length > 0 ? "function-based" : "none",
      },
      isFirstGeneration:
        currentCode.html === "" &&
        currentCode.css === "" &&
  ...
  analyzeIntent(message) {
    ...
    if (
      msg.includes("add") &&
      (msg.includes("field") || msg.includes("input") || msg.includes("button"))
    ) {
    ...
    if (msg.includes("style") || msg.includes("color") || msg.includes("css")) {
      return "styling";
    }
    if (
      msg.includes("button") ||
  ...
  findRelevantSections(message, currentCode, analyses) {
    ...
    if (currentCode.js) {
      analyses.jsAnalysis.functions.forEach((func) => {
        if (
          ...
          (msg.includes("form") && func.name.toLowerCase().includes("form")) ||
          (msg.includes("submit") && func.name.toLowerCase().includes("submit"))
        ) {
    ...
    console.log("✅ [ContextBuilder] Relevant sections found:", sections);
    return sections;
  }
...
class ProtocolParser {
  ...
  parseResponse(response) {
    ...
    try {
      ...
      console.error("❌ [ProtocolParser] Structured JSON parse failed:", error);
      console.log("📄 [ProtocolParser] Full response that caused error:");
      ...
      if (extractedJSON) {
        console.log("✅ [ProtocolParser] Legacy JSON extraction successful");
        return this.validateStructure(extractedJSON);
      }

      console.log("🔧 [ProtocolParser] Falling back to unstructured parsing");
  ...
  extractJSON(response) {
    ...
    if (jsonStart !== -1 && jsonEnd !== -1) {
      try {
        ...
        console.log("❌ [ProtocolParser] Strategy 1 failed:", error.message);
      }
    }
    ...
    if (beforeCodeBlock.includes("{")) {
      try {
        ...
        if (jsonMatch) {
          console.log("🎯 [ProtocolParser] Found JSON before code blocks");
          return JSON.parse(jsonMatch[0]);
        }
      } catch (error) {
        console.log("❌ [ProtocolParser] Strategy 2 failed:", error.message);
      }
    }
    ...
    try {
      ...
      if (jsonMatch) {
        console.log("🎯 [ProtocolParser] Found JSON with cleaning strategy");
        return JSON.parse(jsonMatch[0]);
      }
    } catch (error) {
      console.log("❌ [ProtocolParser] Strategy 3 failed:", error.message);
    }

    console.log("❌ [ProtocolParser] All JSON extraction strategies failed");
    return null;
  }
  ...
  validateStructure(parsed) {
    ...
    if (parsed.changes) {
      if (parsed.changes.html) validated.changes.html = parsed.changes.html;
      if (parsed.changes.css) validated.changes.css = parsed.changes.css;
  ...
  parseUnstructuredResponse(response) {
    ...
    if (replacementInstructions.length > 0) {
      console.log("🎯 [ProtocolParser] Found string replacement instructions");
      return {
    ...
    if (htmlMatches) {
      htmlMatches.forEach((match) => {
        ...
        const changeType = this.detectChangeType("html", content, response);
        changes.html.push({
          type: changeType,
          content: content,
    ...
    if (cssMatches) {
      cssMatches.forEach((match) => {
        ...
        const changeType = this.detectChangeType("css", content, response);
        changes.css.push({
          type: changeType,
          content: content,
    ...
    // Extract JS blocks
    const jsMatches = response.match(/```(?:javascript|js)\n([\s\S]*?)\n```/gi);
    if (jsMatches) {
      jsMatches.forEach((match) => {
        const content = match.replace(/```(?:javascript|js)\n|\n```/gi, "");
        const changeType = this.detectChangeType("js", content, response);
        changes.js.push({
          type: changeType,
          content: content,
    ...
    console.log("✅ [ProtocolParser] Unstructured parsing complete:", result);
    return result;
  }
  ...
  extractExplanation(response) {
    ...
    if (beforeCode && beforeCode.length > 10) {
      return (
        beforeCode.substring(0, 200) + (beforeCode.length > 200 ? "..." : "")
      );
    }
...
class StringReplacementEngine {
  ...
  async applyReplacements(instructions, currentCode) {
    console.log("🔧 [StringReplacement] Applying replacement instructions...");
    console.log(
    ...
    for (const instruction of instructions) {
      try {
        const result = await this.applyInstruction(instruction, updatedCode);
        ...
        if (result.success) {
          ...
          changeLog[instruction.target_type].push({
            type: "string_replacement",
            description: instruction.description || "Code replacement applied",
          ...
          console.error(`❌ [StringReplacement] Failed: ${result.error}`);
          changeLog[instruction.target_type].push({
            type: "string_replacement_failed",
            description: instruction.description || "Code replacement failed",
            error: result.error,
            success: false,
          });
        }
  ...
  async applyInstruction(instruction, currentCode) {
    console.log("🎯 [StringReplacement] Applying instruction:", instruction);
  ...
  validateInstruction(instruction) {
    ...
    if (
      ...
      console.log("❌ [StringReplacement] old_string validation failed:", {
    ...
    if (!instruction.new_string || typeof instruction.new_string !== "string") {
      return {
        valid: false,
...
class ValidationEngine {
  ...
  async validateChanges(changeRequest, currentCode) {
    console.log("🛡️ [ValidationEngine] Starting validation pipeline...");
    ...
    try {
      ...
      const semanticResult = this.validateSemantics(changeRequest, currentCode);
      results.details.semantic = semanticResult;

      // Compile results
      results.valid =
        protocolResult.valid && syntaxResult.valid && semanticResult.valid;
      results.errors = [
  ...
  validateProtocol(changeRequest) {
    console.log("🔍 [ValidationEngine] Layer 1: Protocol validation...");
    ...
    console.log(
      `${result.valid ? "✅" : "❌"} [ValidationEngine] Protocol validation:`,
      result
    );
    return result;
  }
  ...
  async validateSyntax(changeRequest) {
    ...
    try {
      ...
      if (changeRequest.changes.html) {
        for (const htmlChange of changeRequest.changes.html) {
          const htmlResult = this.validateHTMLSyntax(htmlChange.content);
          if (!htmlResult.valid) {
            result.errors.push(`HTML syntax error: ${htmlResult.error}`);
            result.valid = false;
          }
        }
      }
    ...
    console.log(
      `${result.valid ? "✅" : "❌"} [ValidationEngine] Syntax validation:`,
      result
    );
    return result;
  }
  ...
  validateSemantics(changeRequest, currentCode) {
    console.log("🔍 [ValidationEngine] Layer 3: Semantic validation...");
...
class ChangeApplicator {
  ...
  async applyChanges(changeRequest, currentCode) {
    ...
    try {
      ...
      const validationResult = await this.validationEngine.validateChanges(
        changeRequest,
        currentCode
      );
      ...
      // Apply HTML changes
      if (changeRequest.changes.html && changeRequest.changes.html.length > 0) {
      ...
      // Apply CSS changes
      if (changeRequest.changes.css && changeRequest.changes.css.length > 0) {
      ...
      // Apply JS changes
      if (changeRequest.changes.js && changeRequest.changes.js.length > 0) {
      ...
      switch (change.type) {
        ...
        case "replace":
          ...
          changes.push({
            ...
            before: oldHTML,
            after: updatedHTML,
            description: actualChanges.description || "Full HTML replacement",
  ...
  applyJSChanges(jsChanges, currentJS) {
    ...
    jsChanges.forEach((change, index) => {
      console.log(`🔄 [ChangeApplicator] JS Change ${index + 1}:`, change.type);
...
const DOM = {
  ...
  resetBtn: null,
  /** @type {HTMLElement} HTML code display */
  htmlCode: null,
...
function initializeApp() {
  ...
  for (const elementName of requiredElements) {
    if (!DOM[elementName]) {
      throw new TypeError(`Required DOM element '${elementName}' not found`);
    }
  }
...
async function processUserMessage(userMessage) {
  ...
  try {
    ...
    if (
      ...
      applicationResult = await stringReplacementEngine.applyReplacements(
        changeRequest.instructions,
        currentCode
      );
    } else {
    ...
    const changesSummary = generateChangesSummary(applicationResult.changeLog);
    const aiResponseText = `${changeRequest.explanation}\n\n${changesSummary}`;
    addMessageToChat(aiResponseText, "ai");

    // Store AI response in chat history
    AppState.chatHistory.push({ role: "assistant", content: aiResponseText });
    ...
    console.error(`❌ [Main] Request #${AppState.requestCount} failed:`, error);
    ...
    logDebug(`Error in request #${AppState.requestCount}: ${error.message}`);
  }
}
...
async function callOpenAIWithNewArchitecture(userMessage, context) {
  ...
  console.log("🎯 [AI] Using structured outputs for reliable JSON responses");
...
function buildSystemPromptWithContext(context) {
  ...
  let prompt = `You are an expert web developer assistant for a Fliplet app. Your job is to help users create and modify HTML, CSS, and JavaScript code reliably.
...
function updateCodeDisplayWithDiffs(changeLog) {
  ...
  if (changeLog.html && changeLog.html.length > 0) {
    ...
    const htmlDiffContent = htmlDiffSection.querySelector(".diff-content");
  ...
  if (changeLog.css && changeLog.css.length > 0) {
    ...
    const cssDiffContent = cssDiffSection.querySelector(".diff-content");
...
function generateChangesSummary(changeLog) {
  ...
  if (changeLog.js && changeLog.js.length > 0) {
    summaryParts.push(`⚡ JavaScript: ${changeLog.js.length} change(s)`);
  }
...
function applyHTMLDiff(currentHTML, diff) {
  ...
  switch (diff.operation) {
      ...
      if (diff.target && diff.content) {
        ...
        if (diff.target.startsWith(".")) {
          ...
          if (selectorMatch) {
            ...
            if (pseudoSelector && pseudoSelector.startsWith("nth-child(")) {
              // Handle nth-child selectors
              const nthMatch = pseudoSelector.match(/nth-child\((\d+)\)/);
              if (nthMatch) {
          ...
          if (targetRegex.test(modifiedHTML)) {
            ...
            console.log("✅ Added content to element with ID:", targetId);
          } else {
            console.warn("⚠️ Target ID not found:", targetId);
          }
        } else {
          // Element-based targeting
          const targetRegex = new RegExp(`(<${diff.target}[^>]*>)`, "i");
          if (targetRegex.test(modifiedHTML)) {
            modifiedHTML = modifiedHTML.replace(
              targetRegex,
              `$1${diff.content}`
      ...
      if (diff.target && diff.content) {
        ...
        if (diff.target.startsWith(".")) {
          ...
          if (modifyRegex.test(modifiedHTML)) {
            ...
            console.warn("⚠️ Target ID not found for modification:", targetId);
          }
        } else {
          const modifyRegex = new RegExp(
            `(<${diff.target}[^>]*>).*?(</${diff.target}>)`,
    ...
    default:
      console.warn("⚠️ Unsupported HTML diff operation:", diff.operation);
  }
...
function applyCSSDiff(currentCSS, diff) {
  console.assert(typeof currentCSS === "string", "currentCSS must be a string");
  console.assert(
    typeof diff === "object" && diff !== null,
    "diff must be an object"
  );
  ...
  if (diff.operation === "modify" && diff.selector && diff.changes) {
    ...
    const normalizedSelector = diff.selector.replace(/\s+/g, " ").trim();
    ...
    if (match) {
      ...
      modifiedCSS = modifiedCSS.replace(
        ...
        (fullMatch, opening, properties, closing) => {
          ...
          // Apply each change
          for (const [property, value] of Object.entries(diff.changes)) {
            ...
            if (propertyRegex.test(newProperties)) {
              ...
              newProperties = newProperties.replace(propertyRegex, newRule);
...
function applyJSDiff(currentJS, diff) {
  console.assert(typeof currentJS === "string", "currentJS must be a string");
  console.assert(
    typeof diff === "object" && diff !== null,
    "diff must be an object"
  );
...
function sanitizeHTMLForPreview(html) {
  ...
  const originalScripts = (html.match(/<script[^>]+src=/gi) || []).length;
  const originalLinks = (html.match(/<link[^>]+rel=["']?stylesheet/gi) || [])
    .length;
...
function updateCodePreview() {
  ...
  try {
    ...
    const rawHTMLContent = AppState.currentHTML || "<p>No HTML content</p>";
